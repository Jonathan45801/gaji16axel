...
const ModalLaporanabsen = ({open,idedit,tanggalf}) => {
    const[editlaporanabsen,Seteditlaporanabsen] = useState({santun:"",keteran:""})
    const caridataabsen =()=>{
        axios.get('/datalaporansantun',{
            params:{
                id:idedit,
                tanggal:tanggalf._d
            }
        }).then((rese)=>{
            for(var a in rese.data)
            {
                if(rese.data.hasOwnProperty(a))
                {
                    Seteditlaporanabsen({...editlaporanabsen,santun:rese.data[a].santun})
                    editlaporanabsen.keteran = rese.data[a].keterangan
                }
            }
    ...
    const updatedatasantunketerangan=()=>{
        Swal.fire({
            ...
            if(res.isConfirmed)
            {
                axios.post('/simpansantunketerangan',{
                    params:{
                        id:idedit,
                        tanggal:tanggalf
                    },
                    santuna:editlaporanabsen.santun,
                    keterangan:editlaporanabsen.keteran,
                }).then((rese)=>{
                   if(rese.data === "sukses")
                   {
...
<div className="relative p-4 w-[49rem] h-full md:h-auto overflow-y-hidden">
    <div className="relative bg-white rounded-lg shadow">
        ...
        <div className="flex justify-end items-center p-6 space-x-2 rounded-b border-t border-gray-400 ">
            ...
            <button type="button" className=" bg-sky-600 hover:bg-gray-700 rounded-md border border-gray-200 text-md font-medium px-5 py-2.5 text-white flex items-center space-x-2" onClick={()=>updatedatasantunketerangan()}>
...
export default ModalLaporanabsen